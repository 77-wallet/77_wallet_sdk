...
impl WalletDomain {
    ...
    pub(crate) async fn validate_password(password: &str) -> Result<(), crate::error::ServiceError> {
    ...
    pub(crate) async fn upgrade_algorithm(password: &str) -> Result<(), crate::error::ServiceError> {
    ...
    pub(crate) async fn restart_existing_wallet(
        ...
        repo: &mut ResourcesRepo,
        address: &str,
    ) -> Result<std::collections::HashSet<u32>, crate::error::ServiceError> {
        // 查询钱包状态并处理重启逻辑
    ...
    pub(crate) async fn check_api_wallet_exist(address: &str) -> Result<bool, crate::error::ServiceError> {
        let pool = crate::context::CONTEXT.get().unwrap().get_global_sqlite_pool()?;
