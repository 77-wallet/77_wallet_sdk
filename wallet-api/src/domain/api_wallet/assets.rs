...
pub(crate) fn wallet_total_assets_query_key(
...
pub(crate) fn wallet_total_assets_query_lock(key: &str) -> Arc<tokio::sync::Mutex<()>> {
...
pub(crate) fn get_cached_wallet_total_assets(key: &str, max_age: Duration) -> Option<BalanceInfo> {
...
pub(crate) fn set_cached_wallet_total_assets(key: &str, value: &BalanceInfo) {
...
impl ApiAssetsDomain {
    // 通过 impl 包装 free function，减少调用侧感知全局静态细节。
    pub(crate) fn wallet_total_assets_query_key(
        wallet_address: Option<&str>,
        account_id: Option<u32>,
        chain_code: Option<&str>,
    ) -> String {
        wallet_total_assets_query_key(wallet_address, account_id, chain_code)
    }

    pub(crate) fn set_cached_wallet_total_assets(key: &str, value: &BalanceInfo) {
        set_cached_wallet_total_assets(key, value)
    }
    ...
    pub async fn get_api_wallet_assets(
        ...
        let cache_key = Self::wallet_total_assets_query_key(wallet_address, account_id, chain_code);
    ...
    pub async fn get_api_wallet_assets_v3(
        ...
        let lock = wallet_total_assets_query_lock(&lock_key);
...
mod tests {
    ...
    use super::{ApiAssetsDomain, load_total_assets_with_cache};
    ...
    async fn total_assets_dedup_allows_single_real_query() {
        ...
        for _ in 0..50 {
            ...
            tasks.push(tokio::spawn(async move {
                load_total_assets_with_cache(&key, Duration::from_secs(5), Duration::from_secs(5), || async move {
    ...
    async fn total_assets_query_returns_stale_when_refresh_fails() {
        ...
        ApiAssetsDomain::set_cached_wallet_total_assets(
            &key,
            &BalanceInfo {
                amount: 7.0,
                currency: "USD".to_string(),
