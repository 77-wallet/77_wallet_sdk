...
use uuid::Uuid;
use wallet_database::{CollectDbPool, repositories::api_wallet::collect::ApiCollectRepo};
...
impl ShadowAdvancer {
    ...
    pub async fn try_advance(&self, trade_no: &str) {
        ...
        for stage in crate::infrastructure::collect::shadow::stage::COLLECT_ADVANCEMENT_ORDER.iter()
        {
            let eval =
        ...
        if let Err(e) = self.intent_tx.try_send(intent) {
            ...
            if let Some(diagnose_tx) = &self.diagnose_tx {
                ...
                if let Some(trade_no) = trade_no {
                    let meta = crate::infrastructure::collect::diagnose::event::DiagnoseMeta::new(
                        trade_no,
                        crate::infrastructure::collect::diagnose::event::DiagnoseSource::Advancer,
                        crate::infrastructure::collect::diagnose::event::DiagnoseStage::Unknown,
                    );
                    if let Err(e) =
...
mod tests {
    ...
    async fn test_try_advance() {
        ...
        let collect_pool = wallet_database::CollectDbPool::new(db_pool);
