...
impl ProcessCollectTx {
    ...
    async fn process_collect_single_tx(
        ...
        if !req.tx_hash.is_empty() {
            ...
            // 1. 查链上是否存在
            match adapter.query_tx_res(&req.tx_hash).await {
                // === A. 链上查到了 ===
                Ok(Some(tx_result)) => {
                    // 修复状态类型问题：将i8转换为bool
                    let is_success = match tx_result.status {
                        ...
                        3 => false,
                        _ => {
                            tracing::warn!(trade_no=%req.trade_no, "collect_tx: 链上交易状态异常: {}", tx_result.status);
                            false
                        }
                    };

                    if is_success {
                        tracing::info!(trade_no=%req.trade_no, "collect_tx: 链上确认成功，直接落成");
                        // 保存nonce值，因为req将被移动
                        let nonce = req.nonce as u64;
                        // 直接标记成功
                        let mock_resp = TransferResp {
                            tx_hash: req.tx_hash.clone(),
                            fee: req.transaction_fee.clone(),
                            consumer: None,
                        };
                        return Self::handle_collect_tx_success(&worker_ctx, req, mock_resp, nonce)
                            .await;
                    } else {
                        tracing::warn!(trade_no=%req.trade_no, "collect_tx: 链上失败，直接标记失败");
                ...
                // === B. 链上没有该hash ===
                Ok(None) => {
                    tracing::info!(trade_no=%req.trade_no, "collect_tx: 链上未找到该交易，准备判断是否需要重发");

                    // B1. 非EVM链 -> 直接重发
                    // 简化处理：基于链码判断是否为EVM链
                    let is_evm_chain = req.chain_code == ChainCode::Ethereum.to_string()
                        || req.chain_code == ChainCode::BnbSmartChain.to_string();

                    if !is_evm_chain {
                        tracing::info!(trade_no=%req.trade_no, "collect_tx: 非EVM链，未找到链上记录，尝试直接广播raw_tx");

                        let raw_tx = req.raw_tx.clone();
                        if raw_tx.is_empty() {
                            // 没raw_tx就只能放弃重新构建
                            tracing::error!(trade_no=%req.trade_no, "collect_tx: 非EVM链，raw_tx为空，无法重发");
                            return Self::handle_collect_tx_failed(
                                &worker_ctx,
                                &req.trade_no,
                                ServiceError::System(SystemError::Internal(
                                    "raw_tx is empty".into(),
                                )),
                            )
                            .await;
                        }
                        ...
                        // 从字符串解析为RawTx对象
                        let raw_tx_obj =
                            wallet_utils::serde_func::serde_from_str::<RawTx>(&raw_tx)?;
                        ...
                        // 直接广播raw_tx
                        match ApiTransDomain::broadcast_transfer(&req.chain_code, raw_tx_obj).await
                        {
                            Ok(tx) => {
                                tracing::info!(trade_no=%trade_no, "collect_tx: 非EVM链 raw_tx 重发成功, tx_hash={}", tx.tx_hash);
                                // 广播成功，直接标记为成功
                                return Self::handle_collect_tx_success(
                                    &worker_ctx,
                                    req,
                                    tx,
                                    nonce,
                            ...
                            Err(err) => {
                                tracing::error!(trade_no=%trade_no, "collect_tx: 非EVM链 raw_tx 重发失败: {}", err);
                                // 广播失败，标记为失败
                                return Self::handle_collect_tx_failed(&worker_ctx, &trade_no, err)
                                    .await;
                            }
                        }
                    }
                    ...
                    // B2. EVM 链 -> 判断 nonce
                    let chain_nonce =
                        ApiTransDomain::nonce(&req.from_addr, &req.chain_code).await?;
                    tracing::info!(trade_no=%req.trade_no, "collect_tx: EVM链 nonce链上={}, 本地={}", chain_nonce, req.nonce);

                    // 1️⃣ 链上 nonce 更大 = 本地 tx 已经过期/被覆盖
                    if chain_nonce > req.nonce as u64 {
                        tracing::warn!(trade_no=%req.trade_no, "collect_tx: nonce已被占用但链上无此hash，判定丢失");
                        return Self::handle_collect_tx_failed(
                            &worker_ctx,
                            &req.trade_no,
                            ServiceError::System(SystemError::Internal("lost pending tx".into())),
                        )
                        .await;
                    }
                    ...
                    if chain_nonce == req.nonce as u64 {
                        tracing::info!(trade_no=%req.trade_no, "collect_tx: nonce一致，本地交易尚未上链，准备重发 raw_tx");

                        let raw_tx = req.raw_tx.clone();
                        if raw_tx.is_empty() {
                            tracing::error!(trade_no=%req.trade_no, "collect_tx: raw_tx为空，无法重发");
                            return Self::handle_collect_tx_failed(
                                &worker_ctx,
                                &req.trade_no,
                                ServiceError::System(SystemError::Internal(
                        ...
                        {
                            Ok(tx) => {
                                tracing::info!(trade_no=%req.trade_no, "collect_tx: 重发raw_tx成功, tx_hash={}", tx.tx_hash);
                                // 广播成功，直接标记为成功
                                let nonce = req.nonce as u64;
                                return Self::handle_collect_tx_success(
                                    &worker_ctx,
                            ...
                            Err(err) => {
                                tracing::warn!(trade_no=%req.trade_no, "collect_tx: 重发raw_tx失败，等待下一轮: {}", err);
                                return Ok(()); // 不立刻失败，让系统再来
                            }
                        }
                    }
                    ...
                    // 最安全：等待下一轮
                    tracing::error!(trade_no=%req.trade_no, "collect_tx: 本地nonce比链上大，可能出现nonce漂移，等待下一轮观察");
                ...
                // === C. RPC 异常 ===
                Err(err) => {
                    tracing::error!(trade_no=%req.trade_no, "collect_tx: 查询链上状态失败: {}", err);
                    return Ok(()); // 容错，下轮再查
                }
            }
        }
