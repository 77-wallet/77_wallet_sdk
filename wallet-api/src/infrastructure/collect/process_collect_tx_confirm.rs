...
impl ProcessCollectTxConfirmReport {
    ...
    pub(super) async fn run(&mut self) {
        ...
        tracing::debug!("closing process collect tx confirm report ------------- end");
    }
        ...
        loop {
            ...
            tokio::select! {
                _ = self.shutdown_rx.recv() => {
                    tracing::debug!("closing process collect tx confirm report -------------------------------");
                    break;
                }
    ...
    fn spawn_single(&self, trade_no: &str) {
        ...
        tracing::debug!(trade_no=%trade_no, "[归集交易确认] 开始处理单个归集交易确认报告");
        ...
        tokio::spawn(async move {
            ...
            tracing::debug!(trade_no=%trade_no, status=%req.status, "[归集交易确认] 查询到交易信息，开始处理确认报告");
    ...
    fn spawn_batch(&mut self) {
        ...
        tracing::debug!("[归集交易确认] 开始批量处理归集交易确认报告");
        ...
        tokio::spawn(async move {
            ...
            tracing::debug!("[归集交易确认] 查询到 {} 笔待处理的归集交易确认报告", collects.len());
            for req in collects {
                let ctx = ctx.clone();
                tokio::spawn(async move {
                    let lock = ctx.get_address_lock(&req.from_addr);
    ...
    async fn process_collect_single_tx_confirm_report(
        ...
        tracing::debug!(trade_no=%req.trade_no, hash=%req.tx_hash, status=%req.status, "[归集交易确认] 开始处理单条归集交易确认报告");

        // 只有在需要检查重试时间时才执行检查
        if check_retry_time {
            let now = chrono::Utc::now();
            let timeout = now - req.updated_at.unwrap();
            tracing::debug!(trade_no=%req.trade_no, "[归集交易确认] 当前时间: {}, 上次更新时间: {}, 超时时间: {}, 当前重试次数: {}", 
            ...
            tracing::debug!(trade_no=%req.trade_no, "[归集交易确认] 直接调用，跳过重试时间检查");
        }
        ...
        tracing::debug!(trade_no=%req.trade_no, "[归集交易确认] 准备调用后端API发送交易事件确认");
        ...
        {
            Ok(_) => {
                tracing::debug!(trade_no=%req.trade_no, "[归集交易确认] 发送交易事件确认成功");
    ...
    async fn handle_confirm_report_success(pool: Arc<sqlx::SqlitePool>, req: ApiCollectEntity) {
        let (next_status, _notes) = if req.status == ApiCollectStatus::Success {
            tracing::debug!(trade_no=%req.trade_no, "[归集交易确认] 交易确认报告上传成功，准备更新状态为ConfirmSuccessReport");
            (ApiCollectStatus::ConfirmSuccessReport, "trans event ack success")
        } else {
            tracing::debug!(trade_no=%req.trade_no, "[归集交易确认] 交易失败确认报告上传成功，准备更新状态为ConfirmFailureReport");
        ...
        match res {
            Ok(_) => {
                tracing::debug!(trade_no=%req.trade_no, "[归集交易确认] 更新交易状态成功，新状态: {}", next_status);
            }
            Err(err) => {
    ...
    async fn handle_confirm_report_failed(
        ...
        match res {
            Ok(_) => {
                tracing::debug!(trade_no=%req.trade_no, "[归集交易确认] 增加重试次数成功，当前重试次数: {}", req.post_confirm_tx_count + 1);
            }
            Err(err) => {
