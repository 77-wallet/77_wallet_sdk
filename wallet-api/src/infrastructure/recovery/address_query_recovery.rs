...
use wallet_database::{
    entities::address_query_state::{AddressQueryStateEntity, AddressQueryStatus}, repositories::api_wallet::address_query_state::AddressQueryStateRepo,
};

use crate::error::service::ServiceError;
use crate::infrastructure::recovery::pool::BackgroundTaskPool;
...
pub async fn scan_and_dispatch(
    ...
    let query_states = if is_startup {
        ...
        let running_states = AddressQueryStateRepo::list_by_status(&pool, AddressQueryStatus::Running).await?;
        let failed_states = AddressQueryStateRepo::list_by_status(&pool, AddressQueryStatus::Failed).await?;
