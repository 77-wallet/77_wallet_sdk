...
impl ProcessWithdrawTxHandle {
    pub(crate) async fn new_with_ctx(
        ...
        let _tx = ProcessWithdrawTx::new(ctx, shutdown_rx1, tx_rx, report_tx);
        // 注释掉自动启动，旧工作者不再运行
        // let handle = tokio::spawn(async move { tx.run().await });

        // 上报交易
        let _tx_report = ProcessWithdrawTxReport::new(ctx, shutdown_rx2, report_rx);
        // 注释掉自动启动，旧工作者不再运行
        ...
        let _tx_confirm_report =
            ProcessWithdrawTxConfirmReport::new(ctx, shutdown_rx3, confirm_report_rx);
        // 注释掉自动启动，旧工作者不再运行
