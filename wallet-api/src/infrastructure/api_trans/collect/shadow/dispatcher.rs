...
impl RunningKey {
    ...
    pub fn from_intent(intent: &CollectIntent) -> Self {
        match intent {
            ...
            CollectIntent::SideEffect(SideEffectIntent::SendResourceOperationTaskAck(trade_no)) => {
                RunningKey::SendResourceOperationTaskAck(trade_no.clone())
            }
        }
    }
}
    ...
    fn from_intent(intent: &CollectIntent, key: RunningKey) -> Self {
        match intent {
            ...
            CollectIntent::SideEffect(SideEffectIntent::SendResourceOperationTaskAck(trade_no)) => {
                Self {
                    worker: "SideEffectWorker",
                    command: "SendResourceOperationTaskAck",
                    phase: "side_effect",
                    trade_no: trade_no.clone(),
                    key,
                    side_effect: true,
...
impl ShadowDispatcher {
    async fn execute_intent(
        ...
        match intent {
            ...
            CollectIntent::SideEffect(SideEffectIntent::SendResourceOperationTaskAck(trade_no)) => {
                info!(trade_no = %trade_no, "Sending SendResourceOperationTaskAck command to SideEffect Worker");
                side_effect_worker
                    .handle(SideEffectCommand::SendResourceOperationTaskAck(trade_no.clone()))
                    .await
            }
        }
    }
