...
impl ShadowScanner {
    ...
    pub async fn scan_round(&self) {
        ...
        self.scan_need_resource_operation_task_ack().await;
    ...
    async fn scan_need_resource_operation_task_ack(&self) {
        trace!(
            max_items = %self.config.max_items_per_scan,
            "Scanning resource operation task ACK records"
        );

        let records = match wallet_database::repositories::api_wallet::resource_operation::ApiResourceOperationRepo::scan_need_task_ack(
            &self.pool,
            self.config.max_items_per_scan,
        ).await {
            Ok(records) => records,
        ...
        for record in records {
            let intent = CollectIntent::SideEffect(SideEffectIntent::SendResourceOperationTaskAck(
                record.resource_trade_no,
            ));
            self.dispatch_intent(intent).await;
        }
