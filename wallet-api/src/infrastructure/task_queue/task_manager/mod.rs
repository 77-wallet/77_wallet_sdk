...
impl TaskManager {
    ...
    pub async fn start_task_check(&self) -> Result<(), crate::ServiceError> {
        ...
        let pool = crate::context::Context::get_global_sqlite_pool().unwrap();
    ...
    async fn check_handle(
        ...
        let manager = crate::context::Context::get_global_task_manager()?;
        ...
        loop {
            ...
            match Self::handle_task(&task).await {
                ...
                Err(e) => {
                    ...
                    if let Ok(pool) = crate::context::Context::get_global_sqlite_pool() {
                        let mut repo =
