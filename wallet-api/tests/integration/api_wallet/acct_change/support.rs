...
use wallet_api::{
    ...
    testkit::mqtt::{api_wallet_pool, core_pool, exec_wallet_order_payload, task_pool},
};
use wallet_database::{
    dao::{
...
pub(super) struct AcctChangeScenario {
...
impl AcctChangeScenario {
    ...
    pub(super) async fn given_api_wallet_sol_usdc_asset_with_symbol_mismatch(
        &self,
        ...
        Ok(AcctChangeFixture::new(msg_id, "sol", token, address, payload))
    }

    pub(super) async fn given_normal_eth_usdt_asset_with_symbol_mismatch(
        &self,
    ) -> Result<AcctChangeFixture> {
        ...
        assert!(
            AssetsDao::get_by_addr_token(pool.as_ref(), "eth", token, address).await?.is_some()
        );
        ...
        Ok(AcctChangeFixture::new(msg_id, "eth", token, address, payload))
    }

    pub(super) async fn given_normal_eth_native_asset_with_missing_token(
        &self,
    ) -> Result<AcctChangeFixture> {
        ensure_eth_chain_active().await?;
        ...
        Ok(AcctChangeFixture::new(msg_id, "eth", "", address, payload))
    }

    pub(super) async fn when_acct_change_payload_executes(
        &self,
    ...
    pub(super) fn then_api_wallet_acct_change_succeeds(&self, status: u8) {
        assert_eq!(status, 2, "ApiWalletAcctChange task should succeed");
    }

    pub(super) fn then_normal_wallet_acct_change_succeeds(&self, status: u8) {
        assert_eq!(status, 2, "AcctChange task should succeed");
    }

    pub(super) fn then_normal_wallet_native_acct_change_succeeds(&self, status: u8) {
        assert_eq!(status, 2, "AcctChange native task should succeed");
    }

    pub(super) async fn then_api_wallet_asset_keeps_usdc_symbol(
        &self,
        fixture: &AcctChangeFixture,
    ) -> Result<()> {
        let api_pool = api_wallet_pool()?;
    ...
    pub(super) async fn then_normal_wallet_token_asset_keeps_usdt_symbol(
        &self,
        fixture: &AcctChangeFixture,
    ) -> Result<()> {
        self.then_normal_wallet_asset_symbol_is(fixture, "USDT").await
    }

    pub(super) async fn then_normal_wallet_native_asset_keeps_eth_symbol(
        &self,
        fixture: &AcctChangeFixture,
    ) -> Result<()> {
        self.then_normal_wallet_asset_symbol_is(fixture, "ETH").await
    }

    async fn then_normal_wallet_asset_symbol_is(
        &self,
        fixture: &AcctChangeFixture,
        expected_symbol: &str,
    ) -> Result<()> {
...
pub(super) struct AcctChangeFixture {
    msg_id: String,
    chain_code: String,
    token: String,
    address: String,
