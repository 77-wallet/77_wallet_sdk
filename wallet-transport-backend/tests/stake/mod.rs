...
async fn test_delegate_is_open() -> Result<(), wallet_transport_backend::Error> {
    ...
    let res = backend_api
        .delegate_is_open(&aes_cbc_cryptor)
        .await
        .unwrap();
    tracing::info!("{res:?}");
...
async fn test_delegate_complete() -> Result<(), wallet_transport_backend::Error> {
    ...
    let res = backend_api
        .delegate_complete(&aes_cbc_cryptor, &order)
        .await
        .unwrap();
    tracing::info!("{res:?}");
...
async fn test_delegate_query_order() -> Result<(), wallet_transport_backend::Error> {
    ...
    let res = backend_api
        .delegate_query_order(&aes_cbc_cryptor, &order)
        .await
        .unwrap();
    tracing::info!("{res:?}");
...
async fn test_delegate_order() -> Result<(), wallet_transport_backend::Error> {
    ...
    let energy = 10000;
    let res = backend_api
        .delegate_order(&aes_cbc_cryptor, &address, energy)
        .await
        .unwrap();
    tracing::info!("{res:?}");
...
async fn test_vote_list() -> Result<(), wallet_transport_backend::Error> {
    ...
    let res = backend_api.vote_list(&aes_cbc_cryptor).await.unwrap();
    tracing::info!("{res:#?}");
