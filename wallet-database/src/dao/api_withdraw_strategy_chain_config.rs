use crate::entities::api_withdraw_strategy_chain_config::ApiWithdrawStrategyChainConfigEntity;
use sqlx::{Executor, Sqlite};

pub(crate) struct ApiWithdrawStrategyChainConfigDao;

impl ApiWithdrawStrategyChainConfigDao {
    ...
    {
        let sql = r#"
            INSERT INTO api_withdraw_strategy_chain_config
        ...
        sqlx::query_as::<_, ApiWithdrawStrategyChainConfigEntity>(sql)
    ...
    {
        let sql = r#"SELECT * FROM api_withdraw_strategy_chain_config WHERE strategy_id = ?"#;
        let result = sqlx::query_as::<_, ApiWithdrawStrategyChainConfigEntity>(sql)
            .bind(strategy_id)
            .fetch_all(exec)
            .await
            .map_err(|e| crate::Error::Database(e.into()))?;
    ...
    where
        E: Executor<'a, Database = Sqlite>,
    {
        let sql = r#"DELETE FROM api_withdraw_strategy_chain_config WHERE strategy_id = ?"#;
        sqlx::query(sql)
            .bind(strategy_id)
            .execute(exec)
            .await
    ...
    where
        E: Executor<'a, Database = Sqlite>,
    {
        let sql = r#"DELETE FROM api_withdraw_strategy_chain_config WHERE strategy_id = ? AND chain_code = ?"#;
        sqlx::query(sql)
            .bind(strategy_id)
            .bind(chain_code)
            .execute(exec)
