...
impl ExpandBatchItemRepo {
    ...
    /// 统计批次下的扩容项数量
    pub async fn count_by_batch_id(pool: &CoreDbPool, batch_id: &str) -> Result<i64, crate::Error> {
        ExpandBatchItemDao::count_by_batch_id(pool.as_ref(), batch_id).await
    }

    /// 统计所有扩容项数量
    pub async fn count_all(pool: &CoreDbPool) -> Result<i64, crate::Error> {
    ...
    /// 获取所有扩容项
    pub async fn get_all(pool: &CoreDbPool) -> Result<Vec<ExpandBatchItemEntity>, crate::Error> {
    ...
    /// 统计批次下的done状态item数量
    pub async fn count_done_items(pool: &CoreDbPool, batch_id: &str) -> Result<i64, crate::Error> {
