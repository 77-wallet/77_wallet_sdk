...
ALTER TABLE api_collect ADD COLUMN resource_gate_result TEXT NULL; -- 资源闸门释放原因，如 resource_ready / fallback_allowed / legacy_bypass / resource_failed
ALTER TABLE api_collect ADD COLUMN resource_block_reason TEXT NULL; -- 当前资源阻塞原因，如 need_platform_delegate / self_resource_insufficient
ALTER TABLE api_collect ADD COLUMN resource_dependency_trade_no TEXT NULL; -- 当前依赖的资源任务号，用于从原交易反查资源代理/回收事实
ALTER TABLE api_collect ADD COLUMN resource_dependency_type TEXT NULL; -- 当前依赖资源任务类型，如 platform_delegate / local_delegate / platform_reclaim / local_reclaim
