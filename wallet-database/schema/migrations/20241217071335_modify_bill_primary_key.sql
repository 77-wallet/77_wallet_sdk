...
ALTER TABLE bill RENAME TO bill_old;

DROP INDEX IF EXISTS bill_hash_idx;
...
CREATE TABLE
    bill (
        ...
        PRIMARY KEY (hash,transfer_type,owner)
    );
...
INSERT INTO bill (
    hash, chain_code, symbol, transfer_type, tx_kind, owner, from_addr, to_addr,
    token, value, transaction_fee, resource_consume, transaction_time, status,
    is_multisig, block_height, queue_id, notes, created_at, updated_at
)
SELECT
    hash, chain_code, symbol, transfer_type, tx_kind, owner, from_addr, to_addr,
    token, value, transaction_fee, resource_consume, transaction_time, status,
