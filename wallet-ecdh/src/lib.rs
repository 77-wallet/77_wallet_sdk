...
mod tests {
    ...
    fn test_derive_ecdsa_from_shared_secret() {
        ...
        // 2. 从共享密钥派生 ECDSA 密钥对
        let (alice_ecdsa_private, alice_ecdsa_public) =
            derive_ecdsa_from_shared_secret(&shared_secret);
        ...
        // 4. 打印派生的 ECDSA 密钥
        println!(
            "Derived ECDSA Private Key (hex): 0x{}",
            hex::encode(alice_ecdsa_private.to_bytes())
        );
